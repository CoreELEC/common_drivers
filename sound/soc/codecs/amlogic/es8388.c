...
static irqreturn_t hp_det_irq_handler(int irq, void *dev_id)
{
	...
	if (ret < 0) {
		ERR("%s: irq_set_irq_type(%d, %d) failed\n", __func__, irq, type);
		return -1;
	}
	else
		ERR("%s: irq_set_irq_type(%d, %d) OK!\n", __func__, irq, type);
...
//	BUG_ON(component->volatile_register);
	...
	if (ret != 2) {
		ERR("es8388_write error->[REG-0x%02x,val-0x%02x]\n",
				reg, value);
		return -1;
	}
		...
		int clk_id, unsigned int freq, int dir)
{
  ...
  DBG("Enter::%s----%d\n",__FUNCTION__,__LINE__);
	DBG("es8388_set_dai_sysclk freq = %d!\n",freq);
...
//	int ret2,i;
	...
	srate = snd_soc_component_read(component, ES8388_IFACE) & 0x80;
	adciface = snd_soc_component_read(component, ES8388_ADC_IFACE) & 0xE3;
	daciface = snd_soc_component_read(component, ES8388_DAC_IFACE) & 0xC7;

	temp = params_rate(params);
	DBG("%s params_rate(params) = %d ! es8388->sysclk = %d !\n", __func__,temp,es8388->sysclk);

//	es8388->sysclk = 12288000;
	coeff = get_coeff(es8388->sysclk, params_rate(params));
	DBG("coeff = %d es8388->sysclk = %d \n",coeff,es8388->sysclk);
...
/*
	for(i=0;i<0x37;i++)
	{
		ret2 = snd_soc_component_read(component, i);
		DBG("%x = %x \n",i,ret2);
		msleep(50);
	}
*/
...
static int es8388_mute(struct snd_soc_dai *dai, int mute, int direction)
{
	//struct snd_soc_component *component = dai->component;
	//struct es8388_priv *es8388 = es8388_private;
	// u16 mute_reg = snd_soc_component_read(component, ES8388_DACCONTROL3) & 0xfb;
...
//		return ret;
    ...
    */
	...
	snd_soc_component_write(component, ES8388_CHIPPOWER, 0x00);
	snd_soc_component_write(component, ES8388_ADCPOWER, 0x59);
	DBG("%s()es8388_set_bias_level succeed!\n",__FUNCTION__);
	return 0;
}
...
//	snd_soc_codec_set_drvdata(codec, es8388);
	...
	if (ret < 0){
		ERR("es8388 probe error\n");
		return ret;
	}
	else
		DBG("es8388 probe reg:%d value:%d\n",ES8388_DACCONTROL18,reg);
...
/*
	...
	else
	{
	    ...
	    if (ret != 0) {
		    ERR("%s request SPK_CON error", __func__);
		    return ret;
	    }
	...
	else
	{
	    ...
	    if (ret != 0) {
		    ERR("%s request hp_ctl error", __func__);
		    return ret;
	    }
...
*/
	...
	else
	{
		...
		if (ret != 0) {
			ERR("%s request HP_DET error", __func__);
			return ret;
		}
		else
	...
	//	ret = gpio_direction_output(es8388->hp_det_gpio,1);
		...
		if (ret) {
			ERR("%s gpio_direction_input HP_DET error", __func__);
			return ret;
		}
		else
		...
		if(ret == 0){
		    DBG("%s:register ISR (irq=%d)\n", __FUNCTION__,hp_irq);
		}
		else
			ERR("%s:request_irq hp_irq failed\n",__FUNCTION__);
	    }
	}

...
static u32 prime_mode = BIT(0);
module_param(prime_mode, uint, 0664);
MODULE_PARM_DESC(prime_mode, "\n prime_mode\n");
		       ...
		       struct vframe_s *vf)
{
	...
	pr_csc(4, "am_vecm: is_amdv_enable()=%d, is_amdv_on()=%d, dv_policy=%d, dv_mode=%d, dv_format=%d, dv_hdr_policy=%d\n",
		is_amdv_enable(), is_amdv_on(), dv_policy, dv_mode, dv_format, dv_hdr_policy);
	pr_csc(4, "am_vecm: cur_hdr_policy=%d, vd_path=%d, vpp_index=%d, get_hdr_module_status(vd_path, vpp_index)=%d, source_format[vd_path]=%d, target_format[VD1_PATH]=%d, sink_hdr_support(vinfo)=%d, is_video_layer_on(VD2_PATH)=%d\n",
		cur_hdr_policy, vd_path, vpp_index, get_hdr_module_status(vd_path, vpp_index), source_format[vd_path], target_format[VD1_PATH], sink_hdr_support(vinfo), is_video_layer_on(VD2_PATH));
		       ...
		       struct vframe_s *vf)
{
	...
	if (is_vpp1(VD2_PATH) &&
		vd_path == VD2_PATH &&
		vpp_index == VPP_TOP1) {
		vd2_map_top1_policy_process(vinfo, source_format, vd_path, vpp_index, vf);
		goto out;
	...
	if (get_hdr_module_status(vd_path, vpp_index) != HDR_MODULE_ON && cur_hdr_policy != 2) {
		...
		if (source_format[vd_path] == HDRTYPE_MVC) {
			...
			pr_csc(32, "%d %s: vd_path=%d  vpp_index = %d\n",
				...
				vd_path,
				vpp_index);
		} else if (source_format[vd_path] == HDRTYPE_PRIMESL) {
			/* primesl process */
#ifdef CONFIG_AMLOGIC_MEDIA_ENHANCEMENT_PRIME_SL
			if (get_prime_sl_hdr_process_policy(vf) == PRIME_SL_HDR_MODE_2020_TO_709) {
				...
				hdr_process_mode[vd_path] = PROC_HDR_TO_SDR;
				target_format[vd_path] = BT709;
			} else if (get_prime_sl_hdr_process_policy(vf) ==
				PRIME_SL_HDR_MODE_709_TO_2020) {
				/* same as sdr ->hdr */
				sdr_process_mode[vd_path] = PROC_SDR_TO_HDR;
				target_format[vd_path] = BT2020_PQ;
			} else {
				...
				sdr_process_mode[vd_path] = PROC_BYPASS;
				hdr_process_mode[vd_path] = PROC_BYPASS;
				if (get_prime_sl_hdr_process_policy(vf) ==
					PRIME_SL_HDR_MODE_BYPASS_2020)
					target_format[vd_path] = BT2020_PQ;
				else
					target_format[vd_path] = BT709;
			}
			...
			if (target_format[VD1_PATH] == BT2020_HLG) {
				...
				cuva_hdr_process_mode[vd_path] = PROC_CUVA_TO_SDR;
				cuva_hlg_process_mode[vd_path] = PROC_CUVAHLG_TO_SDR;
				if (target_format[VD1_PATH] == BT2020_PQ &&
				    source_format[vd_path] == HDRTYPE_HLG)
					target_format[vd_path] = BT2020_PQ;
				else
					target_format[vd_path] = BT709;
		...
		if (source_format[vd_path] == HDRTYPE_MVC) {
			...
			case HDRTYPE_SDR2020:
				if (is_video_layer_on(oth_path)) {
					...
					/* force sdr->hdr */
					if (sdr_mode == 1 && sink_hdr_support(vinfo)
						& HDR_SUPPORT) {
						/* sdr->hdr */
			...
			case HDRTYPE_HLG:
				/* source HLG */
				if (hdr_mode == 0 && is_video_layer_on(oth_path) &&
				    (target_format[oth_path] == BT2020_PQ ||
				     target_format[oth_path] == BT2020_PQ_DYNAMIC)) {
					/* hlg->hdr */
					hlg_process_mode[vd_path] = PROC_HLG_TO_HDR;
					target_format[vd_path] = BT2020_PQ;
				} else if (hdr_mode == 0 && sink_hdr_support(vinfo) & HLG_SUPPORT) {
					/* hlg->hlg */
					hlg_process_mode[vd_path] = PROC_BYPASS;
					target_format[vd_path] = BT2020_HLG;
				} else if ((hdr_mode == 0 && sink_hdr_support(vinfo) & HDR_SUPPORT) &&
					   (hdr_flag & 0x10)) {
					/* hlg->hdr */
					hlg_process_mode[vd_path] = PROC_HLG_TO_HDR;
					target_format[vd_path] = BT2020_PQ;
			...
			case HDRTYPE_HDR10:
				/* source HDR10 */
				if (hdr_mode == 0 && sink_hdr_support(vinfo) & HDR_SUPPORT) {
					/* hdr bypass */
					hdr_process_mode[vd_path] = PROC_BYPASS;
					target_format[vd_path] = BT2020_PQ;
				} else if (hdr_mode == 0 && sink_hdr_support(vinfo) & HLG_SUPPORT) {
			...
			case HDRTYPE_HDR10PLUS:
				/* source HDR10+ */
				if (hdr_mode == 0 && hdr10_plus_support && !is_video_layer_on(oth_path)) {
					/* hdr+ bypass */
					hdr10_plus_process_mode[vd_path] = PROC_BYPASS;
					target_format[vd_path] = BT2020_PQ_DYNAMIC;
				} else if (hdr_mode == 0 && sink_hdr_support(vinfo) & HDR_SUPPORT) {
...
out:
	pr_csc(4, "am_vecm: sdr_process_mode[vd_path]=%d, hdr_process_mode[vd_path]=%d, hlg_process_mode[vd_path]=%d, hdr10_plus_process_mode[vd_path]=%d, target_format[vd_path]=%d\n",
		sdr_process_mode[vd_path], hdr_process_mode[vd_path], hlg_process_mode[vd_path], hdr10_plus_process_mode[vd_path], target_format[vd_path]);
			 ...
			 enum hdr_type_e *source_type, enum vpp_index_e vpp_index)
{
	...
	/*sbtm*/
	/* if (vinfo->hdr_info.sbtm_info.sbtm_support) */
	if (sbtm_en)
...
{
	//special for t7
	// since in with t7, there will be more difference combination
	...
	case HDRTYPE_PRIMESL:
		...
		if (get_prime_sl_hdr_process_policy(vf) == PRIME_SL_HDR_MODE_2020_TO_709) {
			...
			if (sbtm_en && sbtm_mode) {
				...
				sbtm_convert_process(SBTM_SDR_HDR_PROC, vinfo, &m, 11);
				if (vd_path == VD1_PATH)
					hdr_proc_multi_slices(vf, VD1_HDR, HDR_HDR, /* unsure */
						vinfo, &m, s5_silce_mode, vpp_index);
				else if (vd_path == VD2_PATH)
					hdr_proc(vf, VD2_HDR, HDR_HDR, vinfo, &m, vpp_index);
				else if (vd_path == VD3_PATH)
					hdr_proc(vf, VD3_HDR, HDR_HDR, vinfo, &m, vpp_index);
			...
			if (sbtm_en && sbtm_mode) {
				pr_csc(12, "sbtm:vd_path = %d,PROC_BYPASS--HDR_HDR_PROC\n",
					vd_path);
				sbtm_convert_process(SBTM_HDR_HDR_PROC, vinfo, &m, 11);
				if (vd_path == VD1_PATH)
					hdr_proc_multi_slices(vf, VD1_HDR, HDR_HDR,
						vinfo, &m, s5_silce_mode, vpp_index);
				else if (vd_path == VD2_PATH)
				...
				if (chip_cls_id == TV_CHIP) {
					...
					pr_csc(12, "sbtm: OSD, PROC_BYPASS--HDR_HDR_PROC\n");
					sbtm_convert_process(SBTM_OSD_PROC, vinfo, &m, 11);
		...
		if (vd_path == VD2_PATH &&
			...
			if (gamut_conv_enable)
				gamut_convert_process(
					vinfo, source_type, vd_path, &m, 11, DEST_NONE);
			if (vd_path == VD1_PATH)
			...
			if (sbtm_en && sbtm_mode) {
				...
				if (vd_path == VD1_PATH)
					...
					hdr_proc(vf, VD2_HDR, SDR_HDR, vinfo, &m, vpp_index);
				else if (vd_path == VD3_PATH)
					hdr_proc(vf, VD3_HDR, SDR_HDR, vinfo, &m, vpp_index);
				if (chip_cls_id == TV_CHIP) {
					/*for tv chip need sd2hdr function*/
					hdr_proc(vf, OSD1_HDR, HDR_BYPASS, vinfo, NULL, vpp_index);
					hdr_proc(vf, OSD2_HDR, HDR_BYPASS, vinfo, NULL, vpp_index);
	...
	case HDRTYPE_SDR2020:
		if (vd_path == VD2_PATH &&
			...
			if (vd_path == VD1_PATH)
				...
				hdr_proc(vf, VD2_HDR, SDR_HLG, vinfo, &m, vpp_index);
			else if (vd_path == VD3_PATH)
				hdr_proc(vf, VD3_HDR, SDR_HLG, vinfo, &m, vpp_index);
			hdr_proc(vf, OSD1_HDR, SDR_HLG, vinfo, NULL, vpp_index);
			hdr_proc(vf, OSD2_HDR, SDR_HLG, vinfo, NULL, vpp_index);
			hdr_proc(vf, OSD3_HDR, SDR_HLG, vinfo, NULL, vpp_index);
		} else if (sdr_process_mode[vd_path] == PROC_SDR_AC_SDR) {
			...
			if (vd_path == VD1_PATH)
				...
				hdr_proc(vf, VD2_HDR, SDR_AC_ENH, vinfo, &m,
					vpp_index);
			else if (vd_path == VD3_PATH)
				hdr_proc(vf, VD3_HDR, SDR_AC_ENH, vinfo, &m,
					vpp_index);
			if ((vd_path == VD1_PATH &&
			     !is_video_layer_on(VD2_PATH)) ||
			     (vd_path == VD2_PATH &&

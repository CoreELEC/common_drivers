...
#ifdef CONFIG_AMLOGIC_MEDIA_VIDEOCAPTURE
	ext_frame_capture_poll(cur_dispbuf[0]);
#endif
...
{
	struct vpp_frame_par_s *_cur_frame_par = cur_frame_par[index];
...
static ssize_t vdx_state_show(u32 index, char *buf)
{
	...
	if (layer_info) {
		...
		if (layer_info->vsr_safa_support) {
			...
			len += sprintf(buf + len, "safa preh/v_en %d,%d, h/v ratio:%d, %d\n",
...
static struct amvideo_device_data_s amvideo_s6 = {
	.cpu_type = MESON_CPU_MAJOR_ID_S6_,
	.sr_reg_offt = 0x1e00,
	.sr_reg_offt2 = 0x1f80,
	.layer_support[0] = 1,
	.layer_support[1] = 1,

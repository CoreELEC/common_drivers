...
static int amvideocap_get_input_format(struct vframe_s *vf)
{
	...
	} else if ((vf->type & VIDTYPE_VIU_NV12) == VIDTYPE_VIU_NV12 &&
...
static struct class_attribute amvideocap_attrs[] = {
	__ATTR(config, S_IRUGO | S_IWUSR | S_IWGRP, show_amvideocap_config,
	store_amvideocap_config),
	__ATTR_NULL
};

static struct attribute *amvideocap_class_attrs[] = {
	&amvideocap_attrs[0].attr,
	NULL
};

ATTRIBUTE_GROUPS(amvideocap_class);

static struct class amvideocap_class = {
		.name = MODULE_NAME,
		.class_groups = amvideocap_class_groups,
	};
...
static int amvideocap_probe(struct platform_device *pdev)
{
	...
	{
		pr_warn("failed to create ge2d working queue\n");
		return -EPROBE_DEFER;
	}

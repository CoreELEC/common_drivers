...
/* copy from lib/plist.c */
struct plist_head;

static void ion_plist_check_prev_next(struct list_head *t, struct list_head *p,
				  struct list_head *n)
{
...
static void ion_plist_check_list(struct list_head *top)
{
	struct list_head *prev = top, *next = top->next;

	ion_plist_check_prev_next(top, prev, next);
	while (next != top) {
		prev = next;
		next = prev->next;
		ion_plist_check_prev_next(top, prev, next);
...
static void ion_plist_check_head(struct plist_head *head)
{
	if (!plist_head_empty(head))
		ion_plist_check_list(&plist_first(head)->prio_list);
...
void ion_device_add_heap(struct ion_heap *heap)
{
	...
	ion_plist_add(&heap->node, &dev->heaps);

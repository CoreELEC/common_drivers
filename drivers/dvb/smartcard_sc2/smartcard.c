...
static int sc2_smc_addr(struct platform_device *pdev)
{
	...
	p_smc_hw_base = devm_ioremap(&pdev->dev, res->start,
					     resource_size(res));
	if (!p_smc_hw_base) {
	...
	if (res->start == 0xfe000000) {
		...
		pr_info("sc2 smartcard\n");
	} else {
		pr_info("t5w smartcard\n");
		t5w_smartcard = 1;
	}
	return 0;
}
...
static int smc_dev_init(struct smc_dev *smc, int id)
{
	...
	if (smc->use_enable_pin == 1) {
		...
		if (smc->use_enable_pin) {
			...
			if (!ret) {
				...
				pr_info("cannot find resource \"%s\"\n", buf);
			}
		}
	}
	...
	ret = _set_gpio(smc, &smc->reset_pin, "reset_pin",
		OUTPUT, OUTLEVEL_HIGH);
	if (ret) {
		pr_error("smc reset pin request failed, we can not work now\n");
		return -1;
	}
	ret = of_property_read_u32(smc->pdev->dev.of_node,
		"reset_level", &value);
	...
	if (!smc->detect_pin) {
		...
		if (ret) {
			pr_error("smc detect_pin failed, we can not work\n");
			return -1;
		}
	}
#else /*CONFIG_OF */

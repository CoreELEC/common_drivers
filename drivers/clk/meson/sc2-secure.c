...
static struct clk_hw_onecell_data sc2_hw_onecell_data = {
	.hws = {
		...
		[CLKID_DSU_DYN_CLK]		= &sc2_dsu_clk_dyn.hw,
...
#else
#ifndef MODULE
static int sc2_clkc_secure_init(void)
{
	return platform_driver_register(&sc2_driver);
}
arch_initcall_sync(sc2_clkc_secure_init);
#else
int __init meson_sc2_clkc_secure_init(void)
{
	return platform_driver_register(&sc2_driver);
}
#endif

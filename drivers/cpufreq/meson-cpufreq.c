...
int choose_cpufreq_tables_index(const struct device_node *np, u32 cur_cluster)
{
	...
	if (cpufreq_tables_supply[cur_cluster]) {
		...
		ret = get_cpufreq_table_index(GET_DVFS_TABLE_INDEX,
					      cur_cluster, 0, 0);
		pr_debug("%s:clusterid: %u tables_index %u\n", __func__, cur_cluster, ret);
	}
...
static const struct proc_ops meson_opptable_fops = {
	.proc_open = meson_opptable_open,
	.proc_read = seq_read,
	.proc_release = single_release,
};

static const struct proc_ops meson_maxfreq_fops = {
	.proc_open = meson_maxfreq_open,
	.proc_read = seq_read,
	.proc_write = meson_maxfreq_write,
	.proc_release = single_release,
};
...
static int meson_cpufreq_init(struct cpufreq_policy *policy)
{
	...
	if (IS_ERR(dsu_reg)) {
		pr_debug("[cluster%d]get dsu_regulator failed.\n", cur_cluster);
		dsu_reg = NULL;
	} else {
		pr_debug("[cluster%d]get dsu_regulator succeed.\n", cur_cluster);
	}
	...
	pr_debug("[%s %d][%d %d]\n", __func__, __LINE__,
		 cpu_supply_external_used, dsu_clock_shared);
...
static void meson_cpufreq_register_em(struct cpufreq_policy *policy)
{

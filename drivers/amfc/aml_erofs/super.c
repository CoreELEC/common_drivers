...
static int __nocfi erofs_superblock_csum_verify(struct super_block *sb, void *sbdata)
{
	...
	crc = f_crc32c(~0, dsb, EROFS_BLKSIZ - EROFS_SUPER_OFFSET);
	kfree(dsb);
...
static struct fs_parameter_spec erofs_fs_parameters[] = {
	...
	{
		.name  = "cache_strategy",
		.opt   = Opt_cache_strategy,
		.data  = erofs_param_cache_strategy,
		.type  = NULL, // fs_param_is_enum
		.flags = 0,
	},
				...
				struct fs_parameter *param)
{
	...
	erofs_fs_parameters[2].type = f_fs_param_is_enum;
	erofs_fs_parameters[4].type = f_fs_param_is_enum;
